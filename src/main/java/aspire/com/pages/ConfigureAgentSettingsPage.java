...
public class ConfigureAgentSettingsPage extends GenericPage {
	...
	public void excludeVolumes(String volumeName) {
		...
		waitElementToBeVisible("ExecludVolumeCheckboxForH", CONST_WAIT_LOWER_VALUE);
		sleepTime(2000);
		boolean isExecluded = false;
		if (volumeName.equalsIgnoreCase("c")) {
			if (!getElementByCssSelector("ExecludVolumeCheckboxForC").isSelected()) {
				clickOnElement("ExecludVolumeCheckboxForC");
				isExecluded = true;
			}
		} else if (volumeName.equalsIgnoreCase("E")) {
			if (!getElementByCssSelector("ExecludVolumeCheckboxForE").isSelected()) {
				clickOnElement("ExecludVolumeCheckboxForE");
				isExecluded = true;
			}
		} else if (volumeName.equalsIgnoreCase("h")) {
			if (!getElementByCssSelector("ExecludVolumeCheckboxForH").isSelected()) {
				clickOnElement("ExecludVolumeCheckboxForH");
				isExecluded = true;
			}
		} else if (volumeName.equalsIgnoreCase("I")) {
			if (!getElementByCssSelector("ExecludVolumeCheckboxForI").isSelected()) {
